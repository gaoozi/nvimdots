...
opt.grepprg = 'rg --ignore-case --vimgrep' -- Use ripgrep replace grep
opt.grepformat = '%f:%l:%c:%m,%f:%l:%m'
opt.wildignore = {                         -- Ignore some folders and files with find
  '**/node_modules/**',
  '**/coverage/**',
  '**/.idea/**',
  '**/.git/**',
...
opt.signcolumn = 'yes'
opt.statuscolumn = "%C%=%4{&nu && v:virtnum <= 0 ? (&rnu ? (v:lnum == line('.') ? v:lnum . ' ' : v:relnum) : v:lnum) : ''}%=%s"
